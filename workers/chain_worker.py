...
TRANSACTION_RETRY_BASE_DELAY = 0.5  # seconds
...
class ChainWorker(threading.Thread):
    ...
    def _process_registry_batch(self, batch_start, batch_end):
        ...
        # Process regular registry logs (decoding happens outside transaction)
        for raw in raw_logs:
        ...
        for attempt in range(MAX_TRANSACTION_RETRIES):
            try:
                with self.db.db_session() as session:
                    for name, evt, timestamp, tx_receipt in decoded_events:
                        try:
                            ...
                            raise  # Re-raise to be caught by outer retry
                        except Exception as e:
                            logging.error(
                ...
                if self._is_transient_error(e) and attempt < MAX_TRANSACTION_RETRIES - 1:
                    delay = TRANSACTION_RETRY_BASE_DELAY * (2 ** attempt)
                    logging.warning(
                        f"[Chain {self.chain_id}] Transient error in registry batch, retrying in {delay}s (attempt {attempt + 1}/{MAX_TRANSACTION_RETRIES}): {e}"
                    )
                    time.sleep(delay)
                else:
                    raise
    ...
    def _process_wasm_batch(self, batch_start, batch_end):
        ...
        # Process WASM registry logs (decoding happens outside transaction)
        ...
        for attempt in range(MAX_TRANSACTION_RETRIES):
            try:
                ...
                if self._is_transient_error(e) and attempt < MAX_TRANSACTION_RETRIES - 1:
                    delay = TRANSACTION_RETRY_BASE_DELAY * (2 ** attempt)
                    logging.warning(
                        f"[Chain {self.chain_id}] Transient error in WASM batch, retrying in {delay}s (attempt {attempt + 1}/{MAX_TRANSACTION_RETRIES}): {e}"
                    )
                    time.sleep(delay)
                else:
                    raise
